...
def find_words_with_ligatures(words, lig_regex):
...
def build_ss2lig_map(words_with_ligatures, lig_regex):
    ss2lig = {}
...
def compile_lig_regex(ligatures):
...
class LigatureMap(object):
    def __init__(self, lig_words, ss2lig, ligatures):
        ...
        self.regex = compile_lig_regex(ligatures)
...
def build(words, ligatures=COMMON_LIGATURES):
    ...
    lig_regex = compile_lig_regex(ligatures)
    lig_words = find_words_with_ligatures(words, lig_regex)
    ss2lig = build_ss2lig_map(lig_words, lig_regex)
